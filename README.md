# AWS RDS Service Broker [![Build Status](https://travis-ci.org/AusDTO/pe-rds-broker.png)](https://travis-ci.org/AusDTO/pe-rds-broker)
...
Modify the [included manifest file](manifest.yml) to include your AWS credentials and optionally the [sample configuration file](config-sample.json). Then you can push the broker to your [Cloud Foundry](https://www.cloudfoundry.org/) environment:
...
The Docker image cames with an [embedded sample configuration file](config-sample.json). If you want to override it, you can create the Docker image with you custom configuration file by running:
...
Refer to the [Configuration](https://github.com/CONFIGURATION.md) instructions for details about configuring this broker.

This broker gets the AWS credentials from the environment variables `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`. It requires a user with some [IAM](https://aws.amazon.com/iam/) & [RDS](https://aws.amazon.com/rds/) permissions. Refer to the [iam_policy.json](iam_policy.json) file to check what actions the user must be allowed to perform.
...
Depending on the [broker configuration](CONFIGURATION.md#rds-broker-configuration), Application Depevelopers can send arbitrary parameters on certain broker calls:
...
We use the [GitHub issue tracker](https://github.com/AusDTO/pe-rds-broker/issues) to track bugs and features. Before submitting a bug report or feature request, check to make sure it hasn't already been submitted. You can indicate support for an existing issue by voting it up. When submitting a bug report, please include a [Gist](http://gist.github.com/) that includes a stack trace and any details that may be necessary to reproduce the bug, including your Golang version and operating system. Ideally, a bug report should include a pull request with failing specs.
...
Copyright (c) 2015 Pivotal Software Inc. See [LICENSE](LICENSE) for details.
